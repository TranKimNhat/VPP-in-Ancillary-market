...
class GCNActorPPO(nn.Module):
    def __init__(self, in_dim: int = 10, hidden_dim: int = 64, emb_dim: int = 64, action_dim: int = 123, global_obs_dim: int = 410) -> None:
        ...
        feature_dim = emb_dim + global_obs_dim
    ...
    def _features(
        ...
        x_t = self._to_tensor(node_obs)
        g_t = self._to_tensor(global_obs)
        h = self.encoder(x_t, edge_index)

        if h.ndim == 2:
            pooled = h.sum(dim=0, keepdim=True)
            g_flat = g_t.reshape(1, -1)
        elif h.ndim == 3:
            pooled = h.sum(dim=1)
            g_flat = g_t.reshape(g_t.shape[0], -1)
        else:
    ...
    def dist(
        ...
        pooled, g_flat = self._features(node_obs, edge_index, global_obs)
        z = torch.cat([pooled, g_flat], dim=-1)
...
class GCNNPPOAgent:
    ...
    def _update_model(self) -> dict[str, float]:
        ...
        old_log_probs_all = torch.stack(self.buffer.log_probs).detach()
        ...
        if self.normalize_returns:
            ...
            ret_all = torch.tensor((returns_np - ret_mean) / ret_std, dtype=torch.float32)
        else:
            ret_all = torch.tensor(returns_np, dtype=torch.float32)
        ...
        for _epoch in range(self.n_epochs):
            ...
            for start in range(0, n_transitions, self.mini_batch_size):
                ...
                mb_log_probs = torch.zeros(mb_idx.size, dtype=torch.float32)
                mb_entropy_each = torch.zeros(mb_idx.size, dtype=torch.float32)
                mb_value_pred = torch.zeros(mb_idx.size, dtype=torch.float32)
                ...
                for key, buf_indices in topo_to_mb_indices.items():
                    ...
                    buf_idx_t = torch.as_tensor(buf_indices, dtype=torch.long)
                    ...
                    target_positions = torch.as_tensor(
                        [buf_to_mb_pos[bi] for bi in buf_indices], dtype=torch.long
                    )
    ...
    def rollout_episode(self) -> tuple[float, float]:
        ...
        for _ in range(300):
            ...
            action_env_flat, log_prob, value, raw_action_flat = self.act(
                obs_full, edge_index, global_obs
            )
            ...
            post_freq_state = self.env.freq_dyn.get_state()
            ...
            self.buffer.add(
                obs=torch.tensor(obs_full, dtype=torch.float32),
