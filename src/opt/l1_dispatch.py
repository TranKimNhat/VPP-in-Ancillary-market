...
class L1Dispatcher:
    ...
    def _solve_vpp_lp(
        ...
        Q_commit = cp.Variable(nonneg=True)
        u = cp.Variable()
        ...
        constraints.append(cp.norm(cp.vstack([P_net_abs + R_commit, Q_commit]), 2) <= S_agg)
        P_flex_up = 0.5 * S_agg
        constraints.append(R_commit <= P_flex_up)

        deg_cost = params["deg_cost"]
        price_q = l0_result.lambda_q
        ...
        if prob.status not in {"optimal", "optimal_inaccurate"}:
            return pv_avail, 0.0, 0.0, 0.0, 0.0, soc

        p_val = float(P_net.value)
        r_val = float(R_commit.value)
        q_val = float(Q_commit.value)
        ...
        if p_val < 0:
            ...
            print(f"  q_commit      = {q_val:.4f} Mvar")
            print("=========================================\n")

        soc_next_val = float(np.clip(soc_next.value, 0.1, 0.9))
        revenue_val = lambda_p2p * p_val + price_as * r_val + price_q * q_val
        return p_val, r_val, q_val, p_val, revenue_val, soc_next_val
    ...
    def solve_step(
        ...
        for _iter in range(admm_max_iter):
            ...
            for vpp_id in vpp_ids:
                ...
                p_ref[vpp_id] = p_val
                r_as[vpp_id] = r_val
                q_commit[vpp_id] = q_val
                p_p2p[vpp_id] = p_val
                revenue[vpp_id] = zone_lambda_p2p * p_val + price_as * r_val + price_q * q_val
