...
def main() -> int:
    ...
    axL.barh(y_pos, bill, color=colors, edgecolor=edges, linewidth=1.4,
             height=0.65)
    ...
    for yp, val, sr in zip(y_pos, bill, df["ffr_success_rate"]):
        ...
        axL.text(-bill_max * 0.02, yp, f"SR={sr:.2f}",
                 va="center", ha="right",
                 fontsize=15, color="#444444", fontfamily="monospace")
    axL.set_xlim(-bill_max * 0.22, bill_max * 1.25)
    axL.set_xlabel("DSO gross payment (€ / FFR event)", fontsize=18)
    axL.set_title("(a) Cost the DSO pays", fontsize=20)
    axL.set_yticks(y_pos)
    axL.set_yticklabels(methods, fontsize=16)
    axL.tick_params(axis="x", labelsize=15)
    style_grid(axL, minor=False)
    tighten_spines(axL)

    # ---------------- (b) Committed capacity per event ----------------
    commit = df["committed_mw"].to_numpy(float)
    axR.barh(y_pos, commit, color=colors, edgecolor=edges, linewidth=1.4,
    ...
    for yp, val in zip(y_pos, commit):
        axR.text(val + commit_max * 0.02, yp, f"{val:.3f} MW",
                 va="center", ha="left",
                 fontsize=16, fontweight="bold", color="#222222")
    axR.set_xlim(0, commit_max * 1.30)
    axR.set_xlabel("Avg committed capacity (MW / FFR event)", fontsize=18)
    axR.set_title("(b) Capacity the DSO procures", fontsize=20)
