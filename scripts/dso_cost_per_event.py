...
event as:
    ...
    C_DSO = lambda_cap * R_armed * T          (reserve procurement)
...
def run_one(pol, event):
    ...
    obs_fast, _, _ = env.reset(options={"force_event": copy.deepcopy(event)})
    ...
    for t in range(300):
        ...
        k_peak = max(k_peak, float(np.sum(env._k_droop_last)))   # MW/Hz committed
    ...
    c_cap = LAMBDA_CAP * r_armed * t_event_h
    c_act = LAMBDA_ACT * e_ffr
    out = dict(nadir=float(f_tr.min()), e_ffr=e_ffr, e_shed=e_shed,
               c_cap=c_cap, c_act=c_act)
...
for scen, event in ev.scenarios.items():
    for name, pol in ev.policies.items():
        accs = [run_one(pol, event) for _ in range(N)]
...
df = pd.DataFrame(rows)[["scenario", "method", "nadir", "e_ffr", "e_shed",
                         "c_cap", "c_act", "c_shed", "c_total"] + sweep_cols]
