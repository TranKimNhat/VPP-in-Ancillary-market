...
def _build_env(env_cfg: dict, repo_root: Path) -> GridEnvironment:
    cfg = EnvConfig(
        ...
        vpp_mode=bool(env_cfg.get("vpp_mode", False)),
        mapping_config=dict(env_cfg.get("mappings", {}) or {}),
    ...
    layer1_csv = repo_root / str(signals.get("layer1_pref_csv", "")) if signals.get("layer1_pref_csv") else None
    market_csv = repo_root / str(signals.get("market_signal_csv", "")) if signals.get("market_signal_csv") else None
    dlmp_csv = repo_root / str(signals.get("dlmp_per_bus_csv", "")) if signals.get("dlmp_per_bus_csv") else None
    ...
    return GridEnvironment(
        ...
        mapping_config=cfg.mapping_config,
    )
