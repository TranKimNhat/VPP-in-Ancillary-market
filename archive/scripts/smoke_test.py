...
def test_single_env():
    ...
    missing_keys = sorted(reward_keys - set(breakdown.keys()))
    if missing_keys:
        _fail(f"reward_breakdown missing keys: {missing_keys}; got {sorted(breakdown.keys())}")
...
def test_l0_l1_pipeline():
    ...
    if len(net_data["branches"]) != 120:
        _fail(f"Expected 120 branches, got {len(net_data['branches'])}")
    ...
    print(f"slack_bus: {net_data.get('slack_bus')}")
    print(f"zone_totals: {net_data.get('zone_totals')}")
    ...
    profiles = {
        "pv_pu": 0.7,
        "wind_mw": 8.0,
        "load_z1": float(net_data.get("zone_totals", {}).get(1, 5.0)),
        "load_z2": float(net_data.get("zone_totals", {}).get(2, 6.0)),
        "load_z3": float(net_data.get("zone_totals", {}).get(3, 4.5)),
        "load_z4": float(net_data.get("zone_totals", {}).get(4, 3.0)),
    }
    ...
    print(
        f"L0 status: {l0_result.status}, mode={l0_result.solution_mode}, "
        f"strict_status={l0_result.strict_status}, "
        f"strict_time={l0_result.strict_solve_time * 1000:.0f}ms, "
        f"elastic_time={l0_result.elastic_solve_time * 1000:.0f}ms, "
        f"time: {l0_result.solve_time * 1000:.0f}ms"
    )
    ...
    metadata = json.loads((tmp_dir / "metadata.json").read_text(encoding="utf-8"))
    solver_stats = metadata.get("solver_stats", {})
    strict_success = int(solver_stats.get("strict_success", 0))
    elastic_success = int(solver_stats.get("elastic_success", 0))
    infeasible = int(solver_stats.get("infeasible", 0))
    total_calls = int(solver_stats.get("total_calls", strict_success + elastic_success + infeasible))
    elastic_used_ratio = float(solver_stats.get("elastic_used_ratio", 0.0))

    strict_pct = 100.0 * strict_success / max(total_calls, 1)
    elastic_pct = 100.0 * elastic_success / max(total_calls, 1)
    infeasible_pct = 100.0 * infeasible / max(total_calls, 1)
    health_label = str(solver_stats.get("health", "unknown")).upper()
    print(
        f"L0 solver health: strict={strict_pct:.1f}%, elastic={elastic_pct:.1f}%, "
        f"infeasible={infeasible_pct:.1f}% [{health_label}]"
    )

    if elastic_used_ratio > 0.10:
        _fail(f"elastic_used_ratio {elastic_used_ratio:.1%} > 10% — pipeline invalid")
    if elastic_used_ratio > 0.05:
        print(f"WARN: elastic_used_ratio {elastic_used_ratio:.1%} > 5% — check data/model")
