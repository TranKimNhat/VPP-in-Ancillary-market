...
def _from_mpc_compat(mpc_path: str | Path):
...
class MicrogridEnv(gym.Env):
    ...
    def __init__(
        ...
        net = _from_mpc_compat(fixed_mpc)
    ...
    def _compute_reward(
        ...
        if freq_event_flag:
            ...
            r_freq = -weights.get("freq", 50.0) * (shortfall / (required + 1e-6))
        else:
            r_freq = 0.0
        ...
        total = float(np.clip(total_before_clip / 100.0, -15.0, 2.0))
        ...
        breakdown = {
            ...
            "total_before_clip": float(total_before_clip),
            "total_after_clip": float(total),
        }
