...
class FFRMetrics:
    ...
    # Peak COI RoCoF over a 500 ms sliding window of the sub-step trace (IEEE Std
    # 1547 measurement window). Falls back to the 1 s per-step slope when no hi-res
    # trace is recorded (env.hires_substeps <= 1).
    rocof_max_500ms_hz_s: float = 0.0
    ...
    def to_dict(self) -> dict[str, float]:
        return {
            ...
            "rocof_max_500ms_hz_s": self.rocof_max_500ms_hz_s,
        }


def windowed_rocof(f_hires: np.ndarray, dt: float, window_s: float = 0.5) -> float:
    """Worst-case RoCoF over a sliding window of a hi-res COI trace [Hz/s].

    Mirrors the grid-code RoCoF measurement (IEEE Std 1547 uses a ~500 ms window):
    RoCoF(t) = |f(t+window) - f(t)| / window, reported as the max over the trace.
    With a 1-sample-per-fast-step trace (dt = dt_fast) this degrades to the per-step
    slope, recovering the legacy 1 s metric, so it is always safe to call.
    """
    f = np.asarray(f_hires, dtype=float)
    if f.size < 2 or dt <= 0.0:
        return 0.0
    k = max(1, int(round(window_s / dt)))
    if k >= f.size:
        return abs(f[-1] - f[0]) / (dt * (f.size - 1))
    return float(np.max(np.abs(f[k:] - f[:-k]) / (k * dt)))
...
def compute_ffr_metrics(
    ...
    # ITAE over post_window only (same 50-s window as iae_post).
    # Integrating over the full 270-s episode causes tiny AGC residuals
    # (~0.01 Hz tail) to accumulate hundreds of Hz·s² via time-weighting,
    # dwarfing the actual transient contribution and making ITAE useless.
    post_t = np.arange(len(post_delta_f), dtype=float) * dt
    itae = float(np.trapezoid(post_t * np.abs(post_delta_f), post_t))
...
class FFRTopologyEvaluator:
    ...
    def run_episode(
        ...
        # Peak RoCoF on the IEEE Std 1547 500 ms window of the sub-step COI trace.
        # When hires is off (n_sub<=1) the trace is 1 sample/step, so this returns
        # the legacy 1 s per-step slope.
        metrics.rocof_max_500ms_hz_s = windowed_rocof(
            metrics.f_trace_hires, metrics.dt_hires, window_s=0.5
        )
    ...
    def build_table1_ffr_comparison(self, n_runs: int = 20) -> pd.DataFrame:
        ...
        rows = []
        # Resolve a 0.1 s sub-step COI trace so the 500 ms-window RoCoF
        # (rocof_max_500ms_hz_s) is measurable; the 1 s per-step trace cannot.
        _prev_hires = int(getattr(self.env, "hires_substeps", 0))
        self.env.hires_substeps = max(_prev_hires, 10)
            ...
            for split_name, topo_list in [
                ...
                for topo_idx in topo_list:
                    for _ in range(n_runs):
                        ...
                        metrics_agg["itae"].append(m.itae)
                ...
                rows.append({
                    ...
                    "itae_mean": float(np.mean(metrics_agg["itae"])),
                    "itae_std": float(np.std(metrics_agg["itae"])),
                })
        ...
        if not df.empty and "method" in df.columns:
            ...
            for policy_name in self.policies.keys():
                ...
                if not train_row.empty and not test_row.empty:
                    ...
                    itae_train = float(train_row["itae_mean"].values[0])
                    itae_unseen = float(test_row["itae_mean"].values[0])
                    itae_gap = (itae_unseen - itae_train) / itae_train if itae_train > 1e-9 else float("nan")
                    itae_retention = itae_train / itae_unseen if itae_unseen > 1e-9 else float("nan")

                    df.loc[(df["method"] == policy_name) & (df["topology_split"] == "unseen"), "itae_generalization_gap"] = itae_gap
                    df.loc[(df["method"] == policy_name) & (df["topology_split"] == "unseen"), "itae_retention_ratio"] = itae_retention
        else:
    ...
    def build_table3_severity_scaling(self, n_runs: int = 10) -> pd.DataFrame:
        ...
        # Dense severity sweep 2.0 -> 6.0 MW in 0.5 MW steps, a SINGLE consistent
        # event type (generator trip at bus 97) so the magnitude axis is a clean
        # continuum (the earlier 3-point sweep mixed a load_step at 2 MW with
        # gen_trips at 4/6 MW, which is not a comparable severity axis).
        severity_scenarios = {
            f"{mw:.1f}MW": EventConfig(type="gen_trip", delta_P_mw=-float(mw),
                                       location=97, t_inject=30.0)
            for mw in np.arange(2.0, 6.0 + 1e-9, 0.5)
        }
        ...
        for policy_name, policy in self.policies.items():
            ...
            for tr_idx in self.train_topologies:
                for _ in range(n_runs):
                    ...
                    train_itaes.append(m.itae)
            iae_train_mean = float(np.mean(train_iaes)) if train_iaes else 0.0
            itae_train_mean = float(np.mean(train_itaes)) if train_itaes else 0.0
            ...
            for t_idx in self.test_topologies:
                ...
                itae_test = float(np.mean(itaes))
                deg_pct = ((iae_test - iae_train_mean) / max(iae_train_mean, 1e-6)) * 100.0
                itae_deg_pct = ((itae_test - itae_train_mean) / max(itae_train_mean, 1e-6)) * 100.0
                rows.append({
                    "method": policy_name,
                    ...
                    "itae_train_mean": itae_train_mean,
                    "itae_test": itae_test,
                    "itae_degradation_pct": itae_deg_pct,
                })
    ...
    def build_topology_score_per_scenario(self, n_runs: int = 3) -> pd.DataFrame:
        """Per-scenario topology-robustness means for the radar (Fig. 1).

        Fig. 6 / Table 2 evaluate generalization on S2 only, so the radar's
        Topology spoke was the same scalar in every panel. Here we sweep ALL
        contingency scenarios over the unseen (test) topologies and record the
        mean post-event IAE/ITAE per (scenario, method). fig_radar converts
        min(mean)/mean within each scenario into a 0..1 robustness score.
        """
        if not self.test_topologies:
            print("No test topologies — skipping per-scenario topology score")
            return pd.DataFrame()
        rows: list[dict[str, Any]] = []
        for scen_name, event in self.scenarios.items():
            for policy_name, policy in self.policies.items():
                iaes, itaes = [], []
                for t_idx in self.test_topologies:
                    for _ in range(n_runs):
                ...
                rows.append({
                    "scenario": scen_name,
                    "method": policy_name,
                    "iae_test_mean": float(np.mean(iaes)) if iaes else 0.0,
                    "itae_test_mean": float(np.mean(itaes)) if itaes else 0.0,
                })
        df = pd.DataFrame(rows)
        df.to_csv(self.output_dir / "topology_score_per_scenario.csv", index=False)
        return df
    ...
    def run_all(self, n_runs: int = 20) -> dict[str, pd.DataFrame]:
        ...
        print("\n[+] Per-scenario topology robustness (radar Topology axis)...")
        self.build_topology_score_per_scenario(n_runs=max(n_runs // 5, 3))
