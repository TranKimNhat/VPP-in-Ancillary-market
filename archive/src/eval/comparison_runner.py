...
class _MATD3Wrapper:
    ...
    def _build_obs(self, obs_fast: np.ndarray) -> np.ndarray:
        ...
        freq_state = self._env.freq_dyn_lti.get_state()
...
class BaselineComparison:
    ...
    def run_episode(self, method: Any, force_event: EventConfig, force_topology: int, n_fast_steps: int = 300) -> dict[str, Any]:
        ...
        for _ in range(n_fast_steps):
            ...
            # COI frequency (resolvable system-frequency observable at the 1 s FFR/SFR
            # step; Anderson 1990). Per-GFM inter-machine deviations are sub-second and
            # not resolved at dt=1 s, so the headline uses the COI.
