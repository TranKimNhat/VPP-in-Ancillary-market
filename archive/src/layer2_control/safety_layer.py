...
def enforce_safety(action: np.ndarray, limits: SafetyLimits) -> SafetyResult:
    """Heuristic clip-then-scale safety projection (not exact QP projection)."""
    if action.size < 2:
    ...
    p = float(np.clip(p, limits.p_min, limits.p_max))
    q = float(np.clip(q, limits.q_min, limits.q_max))
    assert limits.p_min - 1e-6 <= p <= limits.p_max + 1e-6
    assert limits.q_min - 1e-6 <= q <= limits.q_max + 1e-6
