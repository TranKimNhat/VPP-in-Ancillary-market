"""DEPRECATED — not used for the reported results.

The proposed GraphSAGE-MAPPO method is trained via ``src/rl/train_am_mappo.py``
(checkpoints ``ckpt_proposed_s42`` / ``am_mappo_final.pt``). This module is kept
for legacy reference only.

NOTE on action handling: the act methods here compute ``log_prob`` on the RAW
Gaussian sample but store/execute the CLIPPED action, which is inconsistent. The
production trainer (``train_am_mappo.py``) clips the sample BEFORE evaluating the
log-probability, so the PPO importance ratio there is scored on the executed
action. Do not use this module as a reference for the policy's log-probability
convention.
"""
...
class MappoPolicy(nn.Module):
    """DEPRECATED legacy MAPPO policy (shared GraphSAGE encoder + centralized critic).

    NOT the trainer behind the reported results — see the module docstring. The
    production policy/trainer is ``src/rl/train_am_mappo.py``. Kept only so the
    legacy ``experiments/train_mappo.py`` / ``experiments/eval_policy.py`` paths
    still import. Do not extend this for new work, and note the raw-vs-clipped
    ``log_prob`` inconsistency documented at module level.
    """

    def __init__(
        self,
        encoder: GraphSAGEEncoder | None = None,
    ...
    def _encode(self, node_features: np.ndarray, edge_index: np.ndarray) -> torch.Tensor:
        return self.encoder.encode(node_features, edge_index)

    def encode(self, node_features: np.ndarray, edge_index: np.ndarray) -> torch.Tensor:
        embeddings = self._encode(node_features, edge_index)
    ...
    def act(self, obs: dict[str, Any]) -> tuple[np.ndarray, float, float]:
        ...
        embeddings = self.encode(node_features, edge_index)
