...
class FeederGraphSAGEAgentEncoder(nn.Module):
    ...
    def forward(self, x_full: torch.Tensor, edge_index: torch.Tensor, agent_bus_idx: torch.Tensor) -> torch.Tensor:
        ...
        # Frequency skip-connection: bypass GNN mean-aggregation for the two
        # time-critical channels (delta_f, rocof). In a radial 123-bus feeder,
        # ~90% of each agent bus's neighbors are load buses with freq=0, so
        # two rounds of mean-aggregation dilute the directional frequency
        # signal — the core cue for deciding P-injection direction. Concatenating
        # the raw unfiltered values restores this signal without changing GNN
        # topology-encoding capacity.
...
class GAMAPPOAgent(nn.Module):
    ...
    def __init__(
        ...
        _actor_in = embed_dim + 2  # +2 for frequency skip-connection (delta_f, rocof)
        self.actor = SharedGaussianActor(
            _actor_in, action_dim_per_agent, hidden_dim,
            log_std_init=log_std_init, min_std=min_std,
        )
        self.critic = AgentCenteredCritic(_actor_in, hidden_dim)
    ...
    def act(
        ...
        obs_t = torch.tensor(obs_full, dtype=torch.float32, device=dev)
        edge_t = torch.tensor(edge_index, dtype=torch.long, device=dev)
...
def train_am_mappo(
    ...
    for ep in range(n_episodes):
        ...
        for _t in range(steps_per_episode):
            ...
            # Reward on the COI frequency deviation. At the 1 s FFR/SFR control step
            # the COI is the resolvable system-frequency observable (Anderson 1990);
            # per-GFM (inter-machine) deviations are a sub-second phenomenon not
            # resolved here, so COI is the correct reward target and keeps train==eval.
        ...
        if len(buffer) > 0:
            ...
            # Prepare batch - per-agent data (on the agent's device)
            _dev = next(agent.parameters()).device
            obs_batch = torch.tensor(np.stack(buffer.obs), dtype=torch.float32, device=_dev)
            edge_batch = [torch.tensor(e, dtype=torch.long, device=_dev) for e in buffer.edge_index]
            actions_batch = torch.tensor(np.stack(buffer.actions), dtype=torch.float32, device=_dev)
            old_log_probs = torch.tensor(np.stack(buffer.log_probs, axis=0), dtype=torch.float32, device=_dev)  # [n_steps, n_agents]
            returns_t = torch.tensor(returns, dtype=torch.float32, device=_dev)  # [n_steps, n_agents]
            advantages_t = torch.tensor(advantages, dtype=torch.float32, device=_dev)  # [n_steps, n_agents]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--device", type=str, default=None,
                        help="cuda | cpu | None(auto). Moves the GraphSAGE agent to the device.")
    parser.add_argument("--skip-to-phase", type=str, default=None,
                        help="Resume curriculum at this phase (e.g. D); earlier phases are skipped.")
    parser.add_argument("--start-global-episode", type=int, default=0,
                        help="Initial global episode counter (for log/filename alignment on resume).")
    parser.add_argument(
        "--ffr-mode",
        type=str,
        default="mappo_dual",
...
def main() -> None:
    ...
    print(f"Environment: {env.n_agents} agents, 3 VPPs, ffr_mode='{env.ffr_mode}', "
          f"fixed_base_topology={env.fixed_base_topology}")
    ...
    # Move agent to the requested device (GraphSAGE encoder + actor + critic).
    device = torch.device(args.device or ("cuda" if torch.cuda.is_available() else "cpu"))
    agent.to(device)
    print(f"[am_mappo] device={device}")

    # Setup curriculum or single-phase training
    if args.curriculum:
        phase_items = list(AM_PHASES.items())
        if args.skip_to_phase is not None:
            phase_items = [(n, c) for (n, c) in phase_items if n >= args.skip_to_phase]
            print(f"[am_mappo] skipping to phase {args.skip_to_phase}; "
                  f"running phases: {[n for n, _ in phase_items]}")
    ...
    global_ep = int(args.start_global_episode)
