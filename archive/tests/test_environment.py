...
def test_environment_vpp_mode_with_unassigned_buses(tmp_path: Path) -> None:
    ...
    for agent_obs in obs.values():
        ...
        assert "edge_index" in agent_obs
        assert "global_state" in agent_obs
        global_state = np.asarray(agent_obs["global_state"], dtype=float)
        edge_index = np.asarray(agent_obs["edge_index"], dtype=np.int64)
        assert global_state.shape == (7,)
        assert np.isfinite(global_state).all()
        assert int(global_state[6]) in {0, 1, 2, 3}
